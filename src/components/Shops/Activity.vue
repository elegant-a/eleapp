<template>
   <div class="rst-activity">
     <div class="activity-txt">
       <span :style="{background:'#' + activites[0].icon_color}">{{activites[0].icon_name}}</span>
       <span>{{activites[0].description}}</span>
     </div>
<!--     共计多少个优惠-->
     <div class="acticity-count" @click="showSheet = true">
       <span>{{activites.length}}个优惠</span>
     ...
     <transition name="fade">
       <div class="act-model" v-show="showSheet">
         <div class="activity-sheet">
           ...
           <h2>优惠活动</h2>
           <ul>
             <li v-for="(item,index) in activites" :key="index">
