...
<!--     订单备注-->
     <div class="view-body">
       <section>
         <textarea v-model="text" placeholder="填写额外对餐厅或者骑手小哥备注的信息" maxlength="100"></textarea>
         <div class="switch" v-for="(item,index) in radioItem" :key="index">
           <span @click="handleRadioItem(item)" :class="{'selected':item.select}"  class="switch-item item-line">{{item.name}}</span>
         </div>
         <div class="switch" v-for="(item,index) in switchItem" :key="index"><span @click="handleRadioItem(item)" :class="{'selected':item.select}" class="switch-item item-line">{{item.name}}</span></div>
       </section>
