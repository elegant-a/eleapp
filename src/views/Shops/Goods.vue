      ...
      <div class="foods-wrapper" ref="foodScroll">
        <ul>
          <li class="food-list-hook" v-for="(item,index) in shopInfo.menu" :key="index">
            ...
            <!--          内容下-->
            <div class="fooddetails" v-for="(food,i) in item.foods" :key="i" @click.self="handleFood(food)">
              <!--            左-->
...
export default {
  ...
  computed: {
    ...
    currentIndex() {
      for (let i = 0; i < this.listHeight.length; i++) {
        ...
        //通过右侧菜单栏的位置和两个height值进行比较如果是在对应的区域,则返回当前区域的i值并与index进行对比
  ...
  methods: {
    ...
    //计算右侧已经滚动的区域区域有多高,计算currentIndex的值
    //最后通过比较currentIndex和index的值来判断給哪个Li添加样式
    calculateHeight() {
      let foodlist = this.$refs.foodScroll.getElementsByClassName("food-list-hook");
      //每个区的高度添加到数组中
      let height = 0;
