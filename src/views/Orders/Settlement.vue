...
<!--        点餐内容-->
        <CartGroup :order-info="orderInfo" :total-price="totalPrice"/>
<!--        备注信息-->
        <section class="checkout-section">
          <cartItem @click="showTableware=true" title="餐具份数" :sub-head="this.$store.getters.remarkInfo.tableware|| '未选择'"/>
          <cartItem @click="$router.push('./remark')" title="订单备注" :sub-head="this.$store.getters.remarkInfo.remark || '口味 偏好'"/>
          <cartItem title="发票信息" sub-head="不需要开发票"/>
        </section>
<!--        显示Tableware-->
        <Tableware :is-show="showTableware" @close="showTableware=false"/>
      </div>
    </div>
